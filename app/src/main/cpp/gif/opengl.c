...
Java_com_bard_gplearning_gif_GifManager_openFile(JNIEnv *env, jobject instance, jstring path_) {
	return openFile(env, path_);
}


JNIEXPORT jint JNICALL
Java_com_bard_gplearning_gif_GifManager_getGifWidth(JNIEnv *env, jobject instance,
													  jlong gifInfo) {
	return getGifWidth(gifInfo);
}

JNIEXPORT jint JNICALL
Java_com_bard_gplearning_gif_GifManager_getGifHeight(JNIEnv *env, jobject instance,
													   jlong gifInfo) {
...
JNIEXPORT jlong JNICALL
Java_com_bard_gplearning_gif_GifManager_renderFrame(JNIEnv *env, jobject instance, jlong gifInfo,
													  jobject jbitmap) {
	GifInfo *info = (GifInfo *) (intptr_t) gifInfo;
	if (info == NULL)
		return -1;
